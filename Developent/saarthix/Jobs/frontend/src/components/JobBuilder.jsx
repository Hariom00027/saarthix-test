...
const JOB_TABS = [
  ...
  { id: 'hrContact', label: 'HR Contact', icon: '👤', required: false }
];
    ...
    try {
      ...
      if (job) {
        setFormData({
          ...
          hrName: job.hrName || '',
          hrContactNumber: job.hrContactNumber || '',
          hrEmail: job.hrEmail || '',
          hrDesignation: job.hrDesignation || '',
          hrDepartment: job.hrDepartment || ''
        });
        setSkillsInput('');
  ...
  const handleSave = async (e) => {
    ...
    try {
      const jobData = {
        ...
        // Optional HR Contact fields
        hrName: formData.hrName || null,
        hrContactNumber: formData.hrContactNumber || null,
        hrEmail: formData.hrEmail || null,
        hrDesignation: formData.hrDesignation || null,
        hrDepartment: formData.hrDepartment || null
      };
      ...
      // Automatically move to next section (except from compensation tab - let Next button handle it)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        <form 
          onSubmit={(e) => {
            // Only submit if on last section (hrContact)
            if (activeTab === 'hrContact') {
              handleSubmit(e);
            } else {
              e.preventDefault();
        ...
        >
          ...
          {activeTab === 'hrContact' && (
            <div className="space-y-6">
              <div className="mb-6 pb-4 border-b border-gray-200">
                <h2 className="text-xl font-semibold text-gray-800">HR Contact Information</h2>
                <p className="text-sm text-gray-500 mt-1">Optional: If provided, this job will be visible in the Placement Access section of SomethingX</p>
              </div>
          ...
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-200">
            ...
            <div className="flex gap-3">
              {/* Conditional Button Rendering - Only one button shows at a time */}
              {activeTab === 'compensation' ? (
                // Next Button - Show on compensation tab to move to HR Contact
                <button
                  type="button"
                  onClick={async (e) => {
                    e.preventDefault();
                    ...
                    try {
                      ...
                      if (jobIdToUpdate) {
                        await axiosInstance.put(`/api/jobs/${jobIdToUpdate}`, jobData);
                      } else {
                      ...
                      // Move to HR Contact tab after saving
                      setActiveTab('hrContact');
                    } catch (err) {
                      console.error('Error saving job:', err);
                    } finally {
                      setSaving(false);
                ...
                // Post Job Button - Show only on HR Contact tab (last section)
                <button
                  type="submit"
                  disabled={saving}
                  className="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-semibold rounded-lg transition-colors text-sm disabled:cursor-not-allowed shadow-md hover:shadow-lg"
                ...
                // Save Button - Show on all other sections
                <button
                  type="button"
                  onClick={handleSave}
                  disabled={saving}
